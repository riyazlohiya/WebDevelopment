 ...
 * Plugin URI: https://github.com/williams_media-sh/example-wordpress-plugin
...
class ExamplePlugin {
  // Replace this with your williams_media account's slug or ID.
  // Available at: https://app.williams_media.sh/settings
  const williams_media_ACCOUNT = 'demo';
  ...
  public function license_key_callback($options) {
    ...
    if (!$res->meta->valid) {
      switch ($res->meta->constant) {
        ...
        // See: https://williams_media.sh/docs/api/#licenses-actions-validate-key-constants.
        default: {
  ...
  // sends a JSON request to williams_media's API, but this could also be your own server
  // which you're using to handle licensing and activation, e.g. something like
  // https://github.com/williams_media-sh/example-php-activation-server.
  private function validate_license_key($key, $domain) {
    $res = wp_remote_post('https://api.williams_media.sh/v1/accounts/' . self::williams_media_ACCOUNT . '/licenses/actions/validate-key', [
      'headers' => [
